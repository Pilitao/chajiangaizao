...
from .Infrastructure import ask_overwrite   # 导入询问函数
...
class COAToolsFormat:
    ...
    def save(self, output_dir="", filename="output"):
        """
        ...
        for wn in self.nodes:
            ...
            try:
                ...
                for idx, child in enumerate(children):
                    sheet_meta = dict()
                    if child.coa != "":
                        # 传递文件名（注意：sprite sheet 仍会生成多个文件，但文件名统一）
                        fn, sheet_meta = child.saveCOASpriteSheet(export_dir, filename)
                    else:
                        fn = child.saveCOA(export_dir, filename)
                    if not fn:
                        continue  # 用户取消覆盖，跳过
                    path = Path(fn)
                    node = child.node
